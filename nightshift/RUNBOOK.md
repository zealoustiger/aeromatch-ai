...
### 2. PM — pick & spec (STRICT priority cascade, human-set 2026-07-05)
The `[goal]` is **GOAL.md: the best set-alert experience site-wide (alerts first).** SEO is
PARKED. Pick exactly ONE task in this **strict order — finish a tier before the next:**

1. **`[bug]` — fix bugs first (uncapped).** Last cycle FAILED, or a known bug / broken page /
   console error / CWV or 375px regression → fix it. A broken **alert / signup / post** flow
   is a P0 bug.
2. **`[want]` — then human-inputted tasks.** Every task the user added to `BACKLOG.md`
...
3. **`[goal]` — then the AI goal: the ALERT EXPERIENCE.** Highest-value `[P1]` slice from
   the 🔔 alert-experience section (alerts before the older activation pillars, which are
   now secondary). `[goal]` = AI-generated goal work.
   ...
   - **All tiers empty →** do NOT invent goal tasks on this cheap model. Emit
     `ABORT — none — plan needed`; the drain runs a **plan pass** on the smart model
     (`config.json.models.plan`, Opus/Fable, via `PLAN_TASK.md`) to generate the next batch.
   - **Check-off is mandatory:** on PASS, mark the item ``✅ SHIPPED via `<slug>` `` in
     `BACKLOG.md` the same cycle; don't add a duplicate for work you just did.
