...
- **[P1][goal] Token-scoped alert management for email-only subscribers.** The core capture promise is "one email field, no account needed" — but `/alerts/manage` requires sign-in (`getUser()`), and the digest email's "Manage alerts" footer links every account-less subscriber straight into that wall (verified: `alert-digest/route.ts` builds a bare `${SITE_URL}/alerts/manage`). Slice: `/alerts/manage?token=<unsubscribe_token>` proves ownership of that alert's email (same public token-scoped precedent as `pauseAlertByToken`) and renders that email's alerts with the existing pause/resume/delete/frequency controls; digest + confirm emails pass the token in their manage links. Improves the manage surface for the majority subscriber (no account); no new capture point, so no new `alert_subscribed` — but this closes the biggest broken loop in the alert lifecycle.
- **[P1][goal] Signed-in one-click alert capture in `AlertSignup`.** Today every alert surface shows an email input even when the visitor is signed in with a verified email — pure retype friction on ~every page of the site at once. When a session exists, render "Alert me — we'll email {their email}" as a single button that creates the alert directly as `status='confirmed'` (ownership already proven by the session — the exact no-second-opt-in precedent `subscribeSavedSearchAlert`/`subscribeToConfirmedAlert` established), falling back to the email field signed-out. One change to the shared component upgrades every existing capture point; still emits `alert_subscribed` (add `signed_in: true` to the payload so the lift is measurable).
- **[P1][goal] Wire `buildPriceDropEmail` into the live digest cron.** The best-in-aviation single-listing price-drop template shipped (`price-drop-email-template`) but is still only reachable via the dev preview route — its stated blocker (the price-drop opt-in toggle) shipped later the same day, so it's now dark for no reason. Slice: in the daily cron pass, when an opted-in aircraft alert's window contains price drops, send the rich per-listing template for the best drop (or top-N cards) instead of/alongside the bare "+1 price drop" count line. Honesty: the send path stays the daily cron — copy must say "dropped this week/yesterday", never "just dropped". No new capture point; completes an already-shipped surface.
- **[P2][goal] Seeker alerts: airport/state in the live match count.** `alertMatchCounts.ts`'s seeker branch still only parses `make`/`model` (verified by code read), so a seeker alert saved with `?airport=KPAO` shows a "N pilots match right now" number on `/alerts/manage` that ignores location — the exact over-matching the digest cron just fixed (`seeker-alert-airport-state`), still live on the manage page. Mirror the cron's state/`home_airport`-OR-`additional_airports` matching in the count query. Honesty fix on an existing surface; flagged as the natural next slice in that cycle's own CHANGELOG note.
- **[P2][goal] Partnership sample cards in the digest email.** `alert-digest-email-redesign` shipped real listing preview cards for aircraft alerts only; partnership alerts still get the CTA-only fallback (flagged "natural follow-up" in that entry). Add up-to-3 partnership preview cards (photo, aircraft, buy-in, share count, airport) via a `fetchNewPartnershipSamples` mirroring the aircraft pattern, honest placeholder-photo caption included. Improves the alert email surface for the partnership half of the marketplace; no new capture point.
- **[P2][goal] Saved-search ↔ alert unification, slice 2: inline alert settings on `/searches`.** After `saved-search-alert-button`, a row shows "✓ Alerts on" but its frequency/price-drop settings live only on `/alerts/manage`. Show the linked alert's Weekly/Daily + price-drop state inline on each alerted saved-search row (reuse `FrequencyToggle`/`PriceDropToggle`), plus a "Turn off alerts (keep the search)" action for symmetry — both flagged as the next slices in that cycle's CHANGELOG. Management-surface improvement; no new capture point.
- **[P2][goal] Contextual alert CTA on `/tools/cost-calculator`.** The calculator collects exactly the context an alert needs (make/model the visitor is pricing out) but has no alert capture at all (verified: no `AlertSignup` under `/tools`). After results render, offer "Get alerts when a {make} {model} is listed" pre-filled from the calculator's inputs, `sourcePath=/aircraft?make=…&model=…` (the matchable shape every other CTA uses). New capture point → emits `alert_subscribed` with the calculator context.
- **[P2][goal] Cross-sell: nearby-state suggestion on `/alerts/status`.** The last un-built suggestion type from the original cross-sell item (make-counterpart and sibling-model both shipped). For a confirmed alert whose criteria include a `state`, offer the adjacent state(s) with actual live inventory ("Also want alerts for {make} in Nevada? 4 listings now") via a small curated state-adjacency map + a real count (reuse `alertMatchCounts` so the suggestion is honest — never offer an empty state). One-click accept via the existing `subscribeToConfirmedAlert`, emitting `alert_subscribed` (`source: 'cross_sell'`).
