...
- **[P1][goal] Alert capture on sold/removed listing pages.** The sold-listing landing page (`SoldListingPage` in `/aircraft/listing/[id]`) is the single highest-intent alert moment on the site — an indexed URL where the buyer wanted *this exact plane* and it's gone — yet it renders only browse links today (confirmed: `AlertSignup` appears only in the active-listing branch). Add an `AlertSignup` block inside/below the amber "no longer for sale" card, prefilled with the family context ("Alert me when another {Make} {Model} lists", `sourcePath=/aircraft?make=…&model=…`, falling back to bare `/aircraft` when no family resolves — honesty gate: never a listing-id-scoped alert that can't match). New capture point → emits `alert_subscribed` with that context/source_path.
- **[P1][goal] Branded 404 page with an alert catch.** There is no `src/app/not-found.tsx` — a dead link or unmatched URL lands on Next's default 404 with zero recovery path. Ship a warm-cream, on-brand not-found page: short copy, links to `/aircraft` + `/partnerships`, and a general `AlertSignup` ("Tell us what you're looking for — we'll email you when it lists", `sourcePath=/`, same pattern as the homepage band). Turns dead ends into an alert entry point sitewide; emits `alert_subscribed`.
- **[P1][goal] One-click "email me about this" on saved searches (saved-search ↔ alert unification, slice 1).** GOAL.md explicitly wants signed-in users to see saved searches and alerts unified; today they're disjoint tables/surfaces. Add a per-row "Get email alerts" button on `/searches` that creates a `status='confirmed'` alert directly for the signed-in user's own verified email (ownership already proven by the session — same no-second-opt-in precedent as `subscribeToConfirmedAlert`), reusing the saved search's query string as `source_path`. Also fix `/account`'s mislabeled "Email alerts" section (it lists saved searches — flagged as a copy follow-up since `alerts-manage-page`) to point at real alerts / `/alerts/manage`. Emits `alert_subscribed` with the saved-search source_path.
- **[P1][goal] Live match count per alert on /alerts/manage.** Each alert row shows only context + status today; a subscriber can't tell if their alert is well-scoped or dead. Show an honest "N listings match right now" per row (server-side count via each alert's existing `parseSourcePath` matching logic — the same query the digest cron runs; show nothing when the source_path isn't parseable rather than a fake 0) with the existing View link. Makes management feel alive and helps users widen/narrow criteria via the shipped Edit form. No new capture point (no event needed); improves the management surface GOAL names as a priority.
- **[P2][goal] Resend-confirmation for pending alerts.** A subscriber whose double-opt-in email got lost is stranded: `/alerts/manage` shows the Pending chip with no action, and the post-subscribe "check your email" state offers no retry. Add a "Resend confirmation email" action (owner-scoped on `/alerts/manage` for pending rows; plus a resend link in `AlertSignup`'s post-submit state), reusing the existing `buildAlertConfirmEmail` send path and the row's existing confirm token — rate-limit to 1 resend per row per ~10 min to stay non-spammy. Recovers otherwise-lost captures at the exact drop-off point.
- **[P2][goal] Seeker alerts: airport/state criteria matching.** Flagged "still not done" since `seeker-alert-make-filter`: `parseSourcePath` matches seeker alerts on `make` only, so a `/partnerships/seeking?airport=…`/`?state=…` alert silently over-matches (fires on seekers anywhere). Extend the digest matcher to honor airport/state for seekers — seeker location is multi-airport (`home_airport` + `additional_airports`) — and have the seeking page's inline `AlertSignup` carry those active filters into `sourcePath`. Honesty fix (alerts should only fire on genuine matches) + a sharper capture on the seeking browse page; capture already emits `alert_subscribed`.
- **[P2][goal] Partnership price-drop alerts (detect + digest count).** Aircraft got price-drop alerts in `aircraft-price-drop-alerts`; partnerships were explicitly deferred ("separate `previous_buy_in_price` column pair — natural next slice"). Mirror the shipped pattern: scraper writes previous buy-in on change, `hasRecentPriceDrop`-style pure helper, `countRecentPartnershipPriceDrops` in the digest cron, counted distinctly in the digest email copy ("1 buy-in drop"), respecting each alert's existing `price_drop_opt_in`. Only fire on genuine decreases; no new capture point.
- **[P2][goal] Cross-sell: sibling-model suggestion on /alerts/status.** Second suggestion type deferred from `alert-cross-sell`: after confirming an aircraft alert with a `model`, offer the sibling model a buyer of that model most commonly also watches (e.g. 172 → 182, SR20 → SR22) via a small curated sibling map seeded from the existing `groupModelVariants`/curated-family data — curated pairs only, no suggestion when the model isn't mapped (honesty gate, same as the shipped make-counterpart type). One-click accept reuses `subscribeToConfirmedAlert` and emits `alert_subscribed` with `source: 'cross_sell'`.
