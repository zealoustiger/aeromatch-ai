...
### Found during `scraper-send-alerts-retire` — 2026-08-27

- **[P1][bug] Saved-search-synced alerts can't be paused or snoozed.** `scraper/sync-saved-searches.mjs` inserts its mirrored rows with `status: 'active'`, and `/api/cron/alert-digest` treats `'active'` as live (`LIVE_ALERT_STATUSES = ['confirmed','active']`), so those rows really do get emailed. But `pauseAlert` / `snoozeAlert` in `src/app/actions.ts` both hard-reject anything that isn't `'confirmed'` ("Only an active alert can be paused."), so a subscriber cannot pause, snooze, or vacation-mode a saved-search alert from `/alerts/manage` or from the digest footer's one-click snooze link. Live today: 3 of the 11 `alerts` rows are `status='active'`. Fix is to make the sync insert `'confirmed'` and backfill the existing `'active'` rows — a consent-surface change, so it wants a human call before it lands.
...
- ~~**[P1][goal] Focus-to-result on `/alerts/manage` row actions (a11y parity).**~~ ✅ SHIPPED via `alert-manage-focus-row-actions` (2026-08-03) After pause/resume/snooze/resend/send-sample, `AlertActions` moves keyboard/SR focus to the row's `role="status"` region (now `tabIndex={-1}`) via a `focusTick`-driven effect (the component stays mounted across the `revalidatePath` refresh); after delete — where the row unmounts — `AlertUndoProvider` focuses the Undo toast (already `role="status"`, now `tabIndex={-1}`) instead of letting focus fall to `<body>`. Reused the existing `role="status"` + `tabindex="-1"` pattern; no schema, no new capture point, no visual change.
...
~~- **[P1][goal] Pause & delete an alert.**~~ ✅ SHIPPED via `alert-pause-delete` (2026-07-06)
  ...
  `status='confirmed'`, so a paused alert is auto-skipped). The legacy
  `scraper/send-alerts.mjs` was not touched.
  **[CORRECTION 2026-08-27]** The claim above that `send-alerts.mjs` was "unwired / not on any
  cron" was WRONG — `nightshift/bin/run-scrape.sh` has run it nightly on the VPS the whole
  time, so it was a *second* live sender racing the Vercel cron over the same `alerts` rows and
  the same `last_digest_at` cursor. Resolved 2026-08-27: its send step is retired and the file
  is now `scraper/sync-saved-searches.mjs` (sync only); `/api/cron/alert-digest` is the sole
  owner of alert email. Also note `alert-digest` selects `status in ('confirmed','active')`,
  not `'confirmed'` alone — and `pauseAlert`/`snoozeAlert` still reject `'active'` rows, so
  saved-search-synced alerts remain unpausable from `/alerts/manage` (open, see below).
...
~~- **[P1][want] Schedule the ingestion scrapers (recurring).**~~ ✅ AUDIT-CONFIRMED FULLY
  ...
  `scraper/ingest-partnerships.mjs` + `scraper/sync-saved-searches.mjs` (formerly
  `send-alerts.mjs`; its send step was retired 2026-08-27 — see the alert-pause-delete
  correction above) daily, unattended. Run the
