...
- **[P1][goal] Detect and store price drops on listings.** Backend slice: add a `price_history` table (new migration) and hook the listing-update path so any price decrease writes a row with old price, new price, and timestamp; expose a `has_recent_price_drop` helper the alert matcher can query. No UI yet — this is the safe foundation slice for price-drop alerts.
- **[P1][goal] Add edit-criteria to alert management.** On `/alerts/manage`, each alert row gets an Edit action opening an inline form pre-filled with the alert's current criteria (make/model/state/price range); saving updates the existing alert record owner-scoped, with a confirmation toast. Uses existing alert columns, no migration needed.
- **[P1][goal] Add alert CTA to partnership detail pages.** On `/partnerships/[id]`, add the same email-only alert block shipped on aircraft listing pages, pre-filled with that partnership's aircraft type and location context, emitting `alert_subscribed` with source `partnership_detail`. Closes the biggest remaining detail-page gap.
- **[P1][goal] Price-drop alert email template.** Build the price-drop notification email (subject, old-vs-new price callout, percent saved, listing photo, manage/unsubscribe links) rendered from the `price_history` data, plus a dev-only preview route to eyeball it; wire it to send when a price drop matches an alert with price-drop enabled (behind a flag if the opt-in toggle isn't live yet).
- **[P1][goal] Price-drop opt-in toggle at capture and manage.** Add an `alert_types` (or `price_drops_enabled`) column via migration, a small "Also alert me on price drops" checkbox (default on) in the existing capture forms, and a matching toggle per alert on `/alerts/manage`. Capture points continue emitting `alert_subscribed` with the chosen types in the event payload.
- **[P1][goal] Digest vs instant frequency choice.** Add a `frequency` column (instant | daily | weekly, migration required) and surface it two places: a subtle "How often?" selector on the post-submit confirmation state of capture forms, and an editable frequency control per alert on `/alerts/manage`. Default instant so capture stays one field.
- **[P2][goal] Rebuild the new-listing alert email to best-in-aviation quality.** Redesign the matched-listing notification email with the warm cream brand tokens: hero photo, price, key specs (year/TTAF/location), a clear "View listing" CTA, "why you got this" criteria echo, and manage-alert footer; add a dev preview route for QA. Content-only, no schema change.
- **[P2][goal] Alert CTA on airport pages.** Add the email-only alert block to airport pages, pre-filled with that airport's location/state context ("Alert me when aircraft or partnerships near KPAO are listed"), emitting `alert_subscribed` with source `airport_page`. Mirrors the shipped make/model and state page pattern.
- **[P2][goal] Post-confirmation cross-sell on /alerts/status.** After a user confirms an alert on `/alerts/status`, show "You're set for X - also want alerts for Y?" with one or two one-click suggestions derived from the confirmed criteria (nearby state, sibling model, or the partnerships counterpart of an aircraft alert); accepting creates the second alert instantly for the already-confirmed email and emits `alert_subscribed` with source `cross_sell`.
- **[P2][goal] Social proof count on capture forms.** Add a live "N buyers get alerts for this model" line (real count from the alerts table, hidden below a threshold of 3) to the listing-detail and make/model capture blocks, with the count included in the `alert_subscribed` event payload so its lift is measurable. Read-only query, no migration.
