...
- **[P1][goal] Alert-count social proof on the `/alerts` landing curated chips.** Each curated popular-alert chip on `/alerts` shows no signal of demand. Where it's honestly ≥ a small threshold of real distinct confirmed subscribers, show a quiet "N pilots watching" line on the chip (reuse the honesty-gated real-count pattern from the listing "Saved by N pilots" social proof — never fabricate, suppress below threshold). Nudges the visitor to set the alert; read-only count, no new capture point.
