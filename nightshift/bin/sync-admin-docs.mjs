 ...
 * on-site admin dashboard always shows the current backlog + latest daily
 * report — even though the report is generated on the work branch (prod and
 * staging share one Supabase database). VENDORED FROM forge/core/bin/sync-admin-docs.mjs.
 *
 * Run from the repo root:  node nightshift/bin/sync-admin-docs.mjs
 * Reads SUPABASE creds from .env.local (or .env).
 */
import { readFileSync, existsSync } from 'node:fs'
import { fileURLToPath } from 'node:url'
import { dirname, join } from 'node:path'
import { estimateMarkdown } from './backlog-estimate.mjs'

const root = join(dirname(fileURLToPath(import.meta.url)), '..', '..')

// Minimal env loader (no dependency): .env.local wins, .env fills gaps.
for (const f of ['.env.local', '.env']) {
  for (const line of existsSync(join(root, f)) ? readFileSync(join(root, f), 'utf8').split('\n') : []) {
