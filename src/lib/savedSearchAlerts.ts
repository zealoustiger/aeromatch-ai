...
import { normalizeFrequency, normalizeDigestDay, type AlertFrequency } from './alertFrequency'
...
export async function getAlertDetailsBySourcePath(email: string): Promise<Map<string, SavedSearchAlertDetail>> {
  ...
  const optionalCols = ['frequency', 'price_drop_opt_in', 'new_listing_opt_out', 'digest_day']
  ...
  // Any subset of frequency/price_drop_opt_in/new_listing_opt_out/digest_day
  // may not be migrated live yet — retry without whichever column(s) the error
  // names (up to four passes), same graceful-degrade pattern as
  // /alerts/manage's fetchAlertsForEmail, rather than losing the page.
