...
export async function generateMetadata(
  ...
  if (!p) return { title: 'Listing not found — AeroMatch' }

  const aircraft = aircraftLabel(p.make, p.model, p.year)
  const desc = p.description?.slice(0, 155) ??
    `${aircraft} ${formatShareType(p.share_type)} at ${p.home_airport}${p.city ? `, ${p.city} ${p.state}` : ''}.`

  return {
    title: `${p.title} — AeroMatch`,
    description: desc,
    openGraph: {
      title: p.title,
      description: desc,
      ...(p.images?.[0] && { images: [{ url: p.images[0] }] }),
    },
  }
}
